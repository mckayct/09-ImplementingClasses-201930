...
class Point(object):
    ...
    def move_to(self,dx,dy):
        self.dist_t = self.dist_t + math.sqrt(((dy - self.y)**2)+((dx - self.x))**2)
        self.x =  dx
        self.y =  dy
    ...
    def move_by(self,dx,dy):
        ...
        self.y = self.y + dy
        self.number_of_moves_made = self.number_of_moves_made + 1
        self.dist_t = self.dist_t + math.sqrt(((dx)**2) +((dy)**2))
    ...
    def get_distance_from_start(self):
        disto = math.sqrt(((self.init_x-self.x)**2)+((self.init_y - self.y)**2))
        return disto
    ...
    def halfway_to(self, Point):
        dist_h_x = (((self.init_x + Point.x)/2))
        dist_h_y = ((self.init_y + Point.y)/2)
        return "Point({}, {})".format(dist_h_x, dist_h_y)
